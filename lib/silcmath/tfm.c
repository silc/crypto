...
/* Original code comes from public domain library tomsfastmath.  Any and
   all changes made to that original code are not in public domain but are
   copyrighted and licensed as defined above. */
...
/* Implementation notes:
   ...
   If result int doesn't have stack set, assure that it doesn't get stack
   in the process.  Often routines tfm_fp_exch temp ints to the result.
   Make sure the temp doesn't deliver stack to the result. */
...
int tfm_fp_init_size(SilcStack stack, tfm_fp_int *a, int size)
{
  ...
  if (stack)
    stack = silc_stack_alloc(size, stack);

  a->alloc = size + 1;
  a->dp = silc_scalloc(stack, a->alloc, sizeof(*a->dp));
  if (!a->dp)
    return TFM_FP_MEM;
  a->used = a->sign = 0;
  a->stack = stack;
...
{
  silc_sfree(a->stack, a->dp);
  a->dp = NULL;
  a->alloc = a->used = a->sign = 0;
  if (a->stack)
    silc_stack_free(a->stack);
}

/* Zeroise */
static inline void tfm_fp_zeroise(tfm_fp_int *a)
{
  if (a->used)
    memset(a->dp, 0, a->used * sizeof(*a->dp));
  a->sign = a->used = 0;
}

/* Grow integer */
int tfm_fp_grow(tfm_fp_int *a, int size)
{
  void *tmp;

  if (!size || a->alloc >= size)
    return TFM_FP_OKAY;

  size += 1;

  SILC_LOG_DEBUG(("Grow %p from %d to %d", a, a->alloc, size));

  tmp = silc_srealloc(a->stack, a->alloc * sizeof(*a->dp), a->dp,
		      sizeof(*a->dp) * size);
  if (!tmp)
    return TFM_FP_MEM;
  a->dp = tmp;

  memset(a->dp + a->alloc, 0, (size - a->alloc) * sizeof(*a->dp));
  a->alloc = size;
...
int tfm_fp_copy(tfm_fp_int *a, tfm_fp_int *b)
{
  ...
  if (b->alloc < a->used) {
    if (tfm_fp_grow(b, a->used))
      return TFM_FP_MEM;
  } else {
    memset(b->dp + a->used, 0, (b->alloc - a->used) * sizeof(*b->dp));
  }

  memcpy(b->dp, a->dp, a->used * sizeof(*a->dp));
...
/* Init and copy.  Use stack as memory if non-NULL. */
int tfm_fp_init_copy(tfm_fp_int *a, tfm_fp_int *b, SilcStack stack)
{
  tfm_fp_sinit(stack, a);
  return tfm_fp_copy(b, a);
}
...
int tfm_fp_2expt(tfm_fp_int *a, int b)
{
  ...
  a->dp[z] = ((tfm_fp_digit)1) << (b % DIGIT_BIT);
  return TFM_FP_OKAY;
}
...
int tfm_fp_add(tfm_fp_int *a, tfm_fp_int *b, tfm_fp_int *c)
{
  ...
  if (sa == sb) {
    ...
    c->sign = sa;
    return s_tfm_fp_add (a, b, c);
  } else {
    /* one positive, the other negative */
    /* subtract the one with the greater magnitude from */
    /* the one of the lesser magnitude.  The result gets */
    /* the sign of the one with the greater magnitude. */
    if (tfm_fp_cmp_mag (a, b) == TFM_FP_LT) {
      c->sign = sb;
      return s_tfm_fp_sub (b, a, c);
    } else {
      c->sign = sa;
      return s_tfm_fp_sub (a, b, c);
    }
  }
}
...
int tfm_fp_addmod(tfm_fp_int *a, tfm_fp_int *b, tfm_fp_int *c, tfm_fp_int *d)
{
  ...
  tfm_fp_init(&tmp);
  if (tfm_fp_add(a, b, &tmp)) {
    tfm_fp_zero(&tmp);
    return TFM_FP_MEM;
  }
  ret = tfm_fp_mod(&tmp, c, d);
  tfm_fp_zero(&tmp);
...
{
   if (a->sign == TFM_FP_NEG && b->sign == TFM_FP_ZPOS) {
      return TFM_FP_LT;
   } else if (a->sign == TFM_FP_ZPOS && b->sign == TFM_FP_NEG) {
      return TFM_FP_GT;
   } else {
      /* compare digits */
      if (a->sign == TFM_FP_NEG) {
         /* if negative compare opposite direction */
         return tfm_fp_cmp_mag(b, a);
      } else {
         return tfm_fp_cmp_mag(a, b);
      }
   }
}
...
int tfm_fp_cmp_d(tfm_fp_int *a, tfm_fp_digit b)
{
  ...
  if ((b && a->used == 0) || a->sign == TFM_FP_NEG) {
    return TFM_FP_LT;
  }
...
{
  tfm_fp_int  q, x, y, t1, t2;
  int     n, t, i, norm, neg;
  SilcStack stack = NULL;

  /* is divisor zero ? */
  if (tfm_fp_iszero (b) == 1) {
    return TFM_FP_VAL;
  }
  ...
  if (tfm_fp_init_size(stack, &t1, 3))
    return TFM_FP_MEM;
  if (tfm_fp_init_size(stack, &t2, 3))
    return TFM_FP_MEM;

  /* q is exchanged with c, use its stack if present. */
  if (tfm_fp_init_size(stack, &q, a->used + 2))
    goto err;
  q.used = a->used + 2;

  /* If result doesn't have stack assure it doesn't get stack in result,
     x is exchanged with d. */
  if (tfm_fp_init_copy(&x, a, d && d->stack ? a->stack : NULL))
    goto err;

  if (tfm_fp_init_copy(&y, b, b->stack))
    goto err;

  /* fix the sign */
  neg = (a->sign == b->sign) ? TFM_FP_ZPOS : TFM_FP_NEG;
  x.sign = y.sign = TFM_FP_ZPOS;

  /* normalize both x and y, ensure that y >= b/2, [b == 2**DIGIT_BIT] */
  norm = tfm_fp_count_bits(&y) % DIGIT_BIT;
  if (norm < (int)(DIGIT_BIT-1)) {
     norm = (DIGIT_BIT-1) - norm;
     if (tfm_fp_mul_2d (&x, norm, &x))
       goto err;
     if (tfm_fp_mul_2d (&y, norm, &y))
       goto err;
  } else {
     norm = 0;
  }
  ...
  if (tfm_fp_lshd (&y, n - t))   /* y = y*b**{n-t} */
    goto err;

  while (tfm_fp_cmp (&x, &y) != TFM_FP_LT) {
    ++(q.dp[n - t]);
    tfm_fp_sub (&x, &y, &x);
  }
  ...
  for (i = n; i >= (t + 1); i--) {
    ...
    if (x.dp[i] == y.dp[t]) {
      q.dp[i - t - 1] = ((((tfm_fp_word)1) << DIGIT_BIT) - 1);
    } else {
      tfm_fp_word tmp;
      tmp = ((tfm_fp_word) x.dp[i]) << ((tfm_fp_word) DIGIT_BIT);
      tmp |= ((tfm_fp_word) x.dp[i - 1]);
      tmp /= ((tfm_fp_word) y.dp[t]);
      q.dp[i - t - 1] = (tfm_fp_digit) (tmp);
    }
    ...
    */
    ...
    do {
      ...
      t1.dp[1] = y.dp[t];
      t1.used = 2;
      if (tfm_fp_mul_d (&t1, q.dp[i - t - 1], &t1))
	goto err;
      ...
      t2.dp[2] = x.dp[i];
      t2.used = 3;
    } while (tfm_fp_cmp_mag(&t1, &t2) == TFM_FP_GT);

    /* step 3.3 x = x - q{i-t-1} * y * b**{i-t-1} */
    if (tfm_fp_mul_d (&y, q.dp[i - t - 1], &t1))
      goto err;
    if (tfm_fp_lshd  (&t1, i - t - 1))
      goto err;
    if (tfm_fp_sub   (&x, &t1, &x))
      goto err;

    /* if x < 0 then { x = x + y*b**{i-t-1}; q{i-t-1} -= 1; } */
    if (x.sign == TFM_FP_NEG) {
      if (tfm_fp_copy (&y, &t1))
	goto err;
      if (tfm_fp_lshd (&t1, i - t - 1))
	goto err;
      if (tfm_fp_add (&x, &t1, &x))
	goto err;
      q.dp[i - t - 1] = q.dp[i - t - 1] - 1;
    }
  }
  ...
  x.sign = x.used == 0 ? TFM_FP_ZPOS : a->sign;
  ...
  if (d != NULL) {
    if (tfm_fp_div_2d (&x, norm, &x, NULL))
      goto err;
...
/* b = a/2 */
int tfm_fp_div_2(tfm_fp_int * a, tfm_fp_int * b)
{
  int     x, oldused;
  ...
  b->used = a->used;
  {
    register tfm_fp_digit r, rr, *tmpa, *tmpb;
...
int tfm_fp_div_2d(tfm_fp_int *a, int b, tfm_fp_int *c, tfm_fp_int *d)
{
  ...
  if (b >= (int)DIGIT_BIT) {
    tfm_fp_rshd (c, b / DIGIT_BIT);
  }

  /* shift any bit count < DIGIT_BIT */
  D = (tfm_fp_digit) (b % DIGIT_BIT);
  if (D != 0) {
    register tfm_fp_digit *tmpc, mask, shift;

    /* mask */
    mask = (((tfm_fp_digit)1) << D) - 1;
...
static int s_is_power_of_two(tfm_fp_digit b, int *p)
{
   int x;
...
int tfm_fp_div_d(tfm_fp_int *a, tfm_fp_digit b, tfm_fp_int *c, tfm_fp_digit *d)
{
  ...
  /* quick outs */
  if (b == 1 || tfm_fp_iszero(a) == 1) {
     if (d != NULL) {
        *d = 0;
     }
     if (c != NULL) {
       return tfm_fp_copy(a, c);
     }
     return TFM_FP_OKAY;
  }
  ...
  if (s_is_power_of_two(b, &ix) == 1) {
     if (d != NULL) {
        *d = a->dp[0] & ((((tfm_fp_digit)1)<<ix) - 1);
     }
     if (c != NULL) {
       return tfm_fp_div_2d(a, ix, c, NULL);
     }
     return TFM_FP_OKAY;
  }

  /* no easy answer [c'est la vie].  Just division */
  if (tfm_fp_init_size(NULL, &q, a->used))
    return TFM_FP_MEM;
  ...
  for (ix = a->used - 1; ix >= 0; ix--) {
     w = (w << ((tfm_fp_word)DIGIT_BIT)) | ((tfm_fp_word)a->dp[ix]);

     if (w >= b) {
        t = (tfm_fp_digit)(w / b);
        w -= ((tfm_fp_word)t) * ((tfm_fp_word)b);
      } else {
        t = 0;
      }
      q.dp[ix] = (tfm_fp_digit)t;
  }
 ...
 */
static int _tfm_fp_exptmod(tfm_fp_int * G, tfm_fp_int * X, tfm_fp_int * P, tfm_fp_int * Y)
{
  tfm_fp_int   M[64], res;
  tfm_fp_digit buf, mp;
  ...
  if ((err = tfm_fp_montgomery_setup (P, &mp)) != TFM_FP_OKAY) {
     return err;
  }

  /* setup result */
  if (tfm_fp_init_size(Y->stack, &res, 1))
    return TFM_FP_MEM;
   ...
   */
   ...
   /* now we need R mod m */
   tfm_fp_montgomery_calc_normalization (&res, P);

   /* now set M[1] to G * R mod m */
   if (tfm_fp_cmp_mag(P, G) != TFM_FP_GT) {
      /* G > P so we reduce it first */
     if (tfm_fp_mod(G, P, &M[1]))
       return TFM_FP_MEM;
   } else {
     if (tfm_fp_copy(G, &M[1]))
       return TFM_FP_MEM;
   }
   if (tfm_fp_mulmod (&M[1], &res, P, &M[1]))
     return TFM_FP_MEM;

  /* compute the value at M[1<<(winsize-1)] by squaring M[1] (winsize-1) times */
  if (tfm_fp_copy (&M[1], &M[1 << (winsize - 1)]))
     return TFM_FP_MEM;
  for (x = 0; x < (winsize - 1); x++) {
    if (tfm_fp_sqr (&M[1 << (winsize - 1)], &M[1 << (winsize - 1)]))
      return TFM_FP_MEM;
    tfm_fp_montgomery_reduce (&M[1 << (winsize - 1)], P, mp);
  }

  /* create upper table */
  for (x = (1 << (winsize - 1)) + 1; x < (1 << winsize); x++) {
    if (tfm_fp_mul(&M[x - 1], &M[1], &M[x]))
      return TFM_FP_MEM;
    tfm_fp_montgomery_reduce(&M[x], P, mp);
  }
  ...
  for (;;) {
    ...
    y     = (tfm_fp_digit)(buf >> (DIGIT_BIT - 1)) & 1;
    buf <<= (tfm_fp_digit)1;
    ...
    if (mode == 1 && y == 0) {
      if (tfm_fp_sqr(&res, &res))
	return TFM_FP_MEM;
      tfm_fp_montgomery_reduce(&res, P, mp);
      continue;
    }
    ...
    if (bitcpy == winsize) {
      ...
      /* then multiply */
      if (tfm_fp_mul(&res, &M[bitbuf], &res))
	return TFM_FP_MEM;
      tfm_fp_montgomery_reduce(&res, P, mp);
...
int tfm_fp_exptmod(tfm_fp_int * G, tfm_fp_int * X, tfm_fp_int * P, tfm_fp_int * Y)
{
   tfm_fp_int tmp;
   int    err;
   ...
   if (X->sign == TFM_FP_NEG) {
     ...
     if (tfm_fp_copy(G, &tmp))
       return TFM_FP_MEM;
     if ((err = tfm_fp_invmod(&tmp, P, &tmp)) != TFM_FP_OKAY) {
       tfm_fp_zero(&tmp);
       return err;
     }
     X->sign = TFM_FP_ZPOS;
     err =  _tfm_fp_exptmod(&tmp, X, P, Y);
     if (X != Y) {
       X->sign = TFM_FP_NEG;
     }
     return err;
     ...
     return _tfm_fp_exptmod(G, X, P, Y);
   }
}
...
int tfm_fp_gcd(tfm_fp_int *a, tfm_fp_int *b, tfm_fp_int *c)
{
   ...
   if (tfm_fp_iszero (a) == 1 && tfm_fp_iszero (b) == 0) {
     tfm_fp_abs (b, c);
     return TFM_FP_OKAY;
   }
   if (tfm_fp_iszero (a) == 0 && tfm_fp_iszero (b) == 1) {
     tfm_fp_abs (a, c);
     return TFM_FP_OKAY;
   }
   ...
   /* sort inputs */
   if (tfm_fp_cmp_mag(a, b) != TFM_FP_LT) {
     /* If result doesn't have stack, assure it doesn't get one */
     if (tfm_fp_init_copy(&u, a, c->stack ? a->stack : NULL))
       return TFM_FP_OKAY;
     if (tfm_fp_init_copy(&v, b, c->stack ? b->stack : NULL))
       return TFM_FP_OKAY;
   } else {
     /* If result doesn't have stack, assure it doesn't get one */
     if (tfm_fp_init_copy(&u, b, c->stack ? b->stack : NULL))
       return TFM_FP_OKAY;
     if (tfm_fp_init_copy(&v, a, c->stack ? a->stack : NULL))
       return TFM_FP_OKAY;
   }

   tfm_fp_init(&r);
   while (tfm_fp_iszero(&v) == TFM_FP_NO) {
     if ((res = tfm_fp_mod(&u, &v, &r)))
       goto out;
     tfm_fp_exch(&v, &u);
     tfm_fp_exch(&r, &v);
   }
...
static int tfm_fp_invmod_slow (tfm_fp_int * a, tfm_fp_int * b, tfm_fp_int * c)
{
  tfm_fp_int  x, y, u, v, A, B, C, D;
  int     res, ret = TFM_FP_MEM;

  /* b cannot be negative */
  if (b->sign == TFM_FP_NEG || tfm_fp_iszero(b) == 1) {
    return TFM_FP_VAL;
  }

  /* init temps */
  tfm_fp_init(&x);    tfm_fp_init(&y);
  tfm_fp_init(&u);    tfm_fp_init(&v);
  tfm_fp_init(&A);    tfm_fp_init(&B);
  tfm_fp_init(&C);    tfm_fp_init(&D);

  /* x = a, y = b */
  if ((res = tfm_fp_mod(a, b, &x)) != TFM_FP_OKAY)
    goto err;
  if (tfm_fp_copy(b, &y))
    goto err;

  /* 2. [modified] if x,y are both even then return an error! */
  if (tfm_fp_iseven (&x) == 1 && tfm_fp_iseven (&y) == 1) {
    return TFM_FP_VAL;
  }
...
top:
  ...
  while (tfm_fp_iseven (&u) == 1) {
    /* 4.1 u = u/2 */
    if (tfm_fp_div_2 (&u, &u))
      goto err;

    /* 4.2 if A or B is odd then */
    if (tfm_fp_isodd (&A) == 1 || tfm_fp_isodd (&B) == 1) {
      /* A = (A+y)/2, B = (B-x)/2 */
      if (tfm_fp_add (&A, &y, &A))
	goto err;
      if (tfm_fp_sub (&B, &x, &B))
  ...
  while (tfm_fp_iseven (&v) == 1) {
    /* 5.1 v = v/2 */
    if (tfm_fp_div_2 (&v, &v))
      goto err;

    /* 5.2 if C or D is odd then */
    if (tfm_fp_isodd (&C) == 1 || tfm_fp_isodd (&D) == 1) {
      /* C = (C+y)/2, D = (D-x)/2 */
      if (tfm_fp_add (&C, &y, &C))
	goto err;
      if (tfm_fp_sub (&D, &x, &D))
  ...
  if (tfm_fp_iszero (&u) == 0)
    goto top;

  /* now a = C, b = D, gcd == g*v */

  /* if v != 1 then there is no inverse */
  if (tfm_fp_cmp_d (&v, 1) != TFM_FP_EQ) {
    ret = TFM_FP_VAL;
    goto err;
  }

  /* if its too low */
  while (tfm_fp_cmp_d(&C, 0) == TFM_FP_LT) {
      if (tfm_fp_add(&C, b, &C))
	goto err;
  }

  /* too big */
  while (tfm_fp_cmp_mag(&C, b) != TFM_FP_LT) {
      if (tfm_fp_sub(&C, b, &C))
	goto err;
  }
...
{
  tfm_fp_int  x, y, u, v, B, D;
  int     neg, ret = TFM_FP_MEM;

  /* 2. [modified] b must be odd   */
  if (tfm_fp_iseven (b) == TFM_FP_YES) {
    return tfm_fp_invmod_slow(a,b,c);
  }

  /* init all our temps */
  tfm_fp_init(&x);  tfm_fp_init(&y);
  tfm_fp_init(&u);  tfm_fp_init(&v);
  tfm_fp_init(&B);  tfm_fp_init(&D);
...
top:
  /* 4.  while u is even do */
  while (tfm_fp_iseven (&u) == TFM_FP_YES) {
    /* 4.1 u = u/2 */
    if (tfm_fp_div_2 (&u, &u))
      goto err;

    /* 4.2 if B is odd then */
    if (tfm_fp_isodd (&B) == TFM_FP_YES) {
      if (tfm_fp_sub (&B, &x, &B))
	goto err;
    }
    /* B = B/2 */
  ...
  /* 5.  while v is even do */
  while (tfm_fp_iseven (&v) == TFM_FP_YES) {
    /* 5.1 v = v/2 */
    if (tfm_fp_div_2 (&v, &v))
      goto err;

    /* 5.2 if D is odd then */
    if (tfm_fp_isodd (&D) == TFM_FP_YES) {
      /* D = (D-x)/2 */
      if (tfm_fp_sub (&D, &x, &D))
	goto err;
    }
  ...
  tfm_fp_zero(&x);  tfm_fp_zero(&y);
  tfm_fp_zero(&u);  tfm_fp_zero(&v);
  tfm_fp_zero(&B);  tfm_fp_zero(&D);
...
int tfm_fp_lcm(tfm_fp_int *a, tfm_fp_int *b, tfm_fp_int *c)
{
   ...
   tfm_fp_init(&t1);
   tfm_fp_init(&t2);
   if ((res = tfm_fp_gcd(a, b, &t1)))
     goto out;
   if (tfm_fp_cmp_mag(a, b) == TFM_FP_GT) {
      if ((res = tfm_fp_div(a, &t1, &t2, NULL)))
	goto out;
      res = tfm_fp_mul(b, &t2, c);
   } else {
      if ((res = tfm_fp_div(b, &t1, &t2, NULL)))
      res = tfm_fp_mul(a, &t2, c);
   }
...
int tfm_fp_mod(tfm_fp_int *a, tfm_fp_int *b, tfm_fp_int *c)
{
   ...
   tfm_fp_init(&t);
   if ((err = tfm_fp_div(a, b, NULL, &t)) != TFM_FP_OKAY) {
      tfm_fp_zero(&t);
      return err;
   }
   if (t.sign != b->sign) {
      err = tfm_fp_add(&t, b, c);
   } else {
      err = TFM_FP_OKAY;
      tfm_fp_exch(&t, c);
   }
...
int tfm_fp_mod_2d(tfm_fp_int *a, int b, tfm_fp_int *c)
{
  ...
  c->dp[b / DIGIT_BIT] &= ~((tfm_fp_digit)0) >> (DIGIT_BIT - b);
  tfm_fp_clamp (c);
  return TFM_FP_OKAY;
}
...
{
   return tfm_fp_div_d(a, b, NULL, c);
}
...
int tfm_fp_montgomery_calc_normalization(tfm_fp_int *a, tfm_fp_int *b)
{
  ...
  bits = tfm_fp_count_bits (b) % DIGIT_BIT;
  if (!bits) bits = DIGIT_BIT;

  /* compute A = B^(n-1) * 2^(bits-1) */
  if (b->used > 1) {
     if (tfm_fp_2expt (a, (b->used - 1) * DIGIT_BIT + bits - 1))
	return TFM_FP_MEM;
  } else {
     if (tfm_fp_set(a, 1))
	return TFM_FP_MEM;
...
/* SSE2 code (assumes 32-bit tfm_fp_digits) */
...
#define INNERMUL                                      \
   do { tfm_fp_word t;                                    \
   _c[0] = t  = ((tfm_fp_word)_c[0] + (tfm_fp_word)cy) +      \
                (((tfm_fp_word)mu) * ((tfm_fp_word)*tmpm++)); \
   cy = (t >> DIGIT_BIT);                             \
   } while (0)

#define PROPCARRY \
   do { tfm_fp_digit t = _c[0] += cy; cy = (t < cy); } while (0)
...
#include "tfm_fp_mont_small.i"
#endif

/* computes x/R == x (mod N) via Montgomery Reduction */
int tfm_fp_montgomery_reduce(tfm_fp_int *a, tfm_fp_int *m, tfm_fp_digit mp)
{
   tfm_fp_digit *c, C[TFM_FP_SIZE], *ac = NULL, *_c, *tmpm, mu;
   int      oldused, x, y, pa;

   /* Grow when needed */
   if (a->alloc < m->used + 1)
     if (tfm_fp_grow(a, m->used + 1))
       return TFM_FP_MEM;

   pa = m->used;

   /* Allocate more space if we're doing huge computations */
   if (2 * pa + 1 >= TFM_FP_SIZE) {
     ac = silc_smalloc(a->stack, (2 * pa + 1) * sizeof(*c));
     if (!ac)
       return TFM_FP_MEM;
     c = ac;
   } else {
  ...
  if (ac)
    silc_sfree(a->stack, ac);

  /* if A >= m then A = A - m */
  if (tfm_fp_cmp_mag (a, m) != TFM_FP_LT) {
    if (s_tfm_fp_sub (a, m, a))
      return TFM_FP_MEM;
  }
  return TFM_FP_OKAY;
}
...
int tfm_fp_montgomery_setup(tfm_fp_int *a, tfm_fp_digit *rho)
{
  tfm_fp_digit x, b;
 ...
 */
  ...
  /* rho = -1/m mod b */
  *rho = (((tfm_fp_word) 1 << ((tfm_fp_word) DIGIT_BIT)) - ((tfm_fp_word)x));
    ...
    */
...
#ifdef TFM_SMALL_SET
        if (y <= 16) {
          /* Grow when needed.  All mul routines expect the space to be there
             and that all integers are same size. */
          if (C->alloc < 32 + 1)
            if (tfm_fp_grow(C, 32 + 1))
	      return TFM_FP_MEM;
          if (A->alloc < 32 + 1)
            if (tfm_fp_grow(A, 32 + 1))
              return TFM_FP_MEM;
          if (B->alloc < 32 + 1)
            if (tfm_fp_grow(B, 32 + 1))
              return TFM_FP_MEM;

           tfm_fp_mul_comba_small(A,B,C);
           return TFM_FP_OKAY;
        }
#endif
        ...
        if (y <= 20) {
          /* Grow when needed.  All mul routines expect the space to be there
             and that all integers are same size. */
          if (C->alloc < 40 + 1)
            if (tfm_fp_grow(C, 40 + 1))
	      return TFM_FP_MEM;
          if (A->alloc < 40 + 1)
            if (tfm_fp_grow(A, 40 + 1))
              return TFM_FP_MEM;
          if (B->alloc < 40 + 1)
            if (tfm_fp_grow(B, 40 + 1))
              return TFM_FP_MEM;

           tfm_fp_mul_comba20(A,B,C);
           return TFM_FP_OKAY;
        }
#endif
        ...
        if (yy >= 16 && y <= 24) {
          /* Grow when needed.  All mul routines expect the space to be there
             and that all integers are same size. */
          if (C->alloc < 48 + 1)
            if (tfm_fp_grow(C, 48 + 1))
	      return TFM_FP_MEM;
          if (A->alloc < 48 + 1)
            if (tfm_fp_grow(A, 48 + 1))
              return TFM_FP_MEM;
          if (B->alloc < 48 + 1)
            if (tfm_fp_grow(B, 48 + 1))
              return TFM_FP_MEM;

           tfm_fp_mul_comba24(A,B,C);
           return TFM_FP_OKAY;
        }
#endif
        ...
        if (yy >= 20 && y <= 28) {
          /* Grow when needed.  All mul routines expect the space to be there
             and that all integers are same size. */
          if (C->alloc < 56 + 1)
            if (tfm_fp_grow(C, 56 + 1))
	      return TFM_FP_MEM;
          if (A->alloc < 56 + 1)
            if (tfm_fp_grow(A, 56 + 1))
              return TFM_FP_MEM;
          if (B->alloc < 56 + 1)
            if (tfm_fp_grow(B, 56 + 1))
              return TFM_FP_MEM;

           tfm_fp_mul_comba28(A,B,C);
           return TFM_FP_OKAY;
        }
#endif
        ...
        if (yy >= 24 && y <= 32) {
          /* Grow when needed.  All mul routines expect the space to be there
             and that all integers are same size. */
          if (C->alloc < 64 + 1)
            if (tfm_fp_grow(C, 64 + 1))
	      return TFM_FP_MEM;
          if (A->alloc < 64 + 1)
            if (tfm_fp_grow(A, 64 + 1))
              return TFM_FP_MEM;
          if (B->alloc < 64 + 1)
            if (tfm_fp_grow(B, 64 + 1))
              return TFM_FP_MEM;

           tfm_fp_mul_comba32(A,B,C);
           return TFM_FP_OKAY;
        }
#endif
        ...
        if (yy >= 40 && y <= 48) {
          /* Grow when needed.  All mul routines expect the space to be there
             and that all integers are same size. */
          if (C->alloc < 96 + 1)
            if (tfm_fp_grow(C, 96 + 1))
	      return TFM_FP_MEM;
          if (A->alloc < 96 + 1)
            if (tfm_fp_grow(A, 96 + 1))
              return TFM_FP_MEM;
          if (B->alloc < 96 + 1)
            if (tfm_fp_grow(B, 96 + 1))
              return TFM_FP_MEM;

           tfm_fp_mul_comba48(A,B,C);
           return TFM_FP_OKAY;
        }
#endif
        ...
        if (yy >= 56 && y <= 64) {
          /* Grow when needed.  All mul routines expect the space to be there
             and that all integers are same size. */
          if (C->alloc < 128 + 1)
            if (tfm_fp_grow(C, 128 + 1))
	      return TFM_FP_MEM;
          if (A->alloc < 128 + 1)
            if (tfm_fp_grow(A, 128 + 1))
              return TFM_FP_MEM;
          if (B->alloc < 128 + 1)
            if (tfm_fp_grow(B, 128 + 1))
              return TFM_FP_MEM;

           tfm_fp_mul_comba64(A,B,C);
           return TFM_FP_OKAY;
        }
#endif
        ...
        /* Grow when needed.  All mul routines expect the space to be there
           and that all integers are same size. */
        if (C->alloc < (y * 2) + 1)
          if (tfm_fp_grow(C, (y * 2) + 1))
            return TFM_FP_MEM;
        if (A->alloc < (y * 2) + 1)
          if (tfm_fp_grow(A, (y * 2) + 1))
            return TFM_FP_MEM;
        if (B->alloc < (y * 2) + 1)
          if (tfm_fp_grow(B, (y * 2) + 1))
            return TFM_FP_MEM;

        return tfm_fp_mul_comba(A,B,C);
}

int tfm_fp_mul_2(tfm_fp_int * a, tfm_fp_int * b)
{
  int     x, oldused;
...

  {
    ...
    for (x = 0; x < a->used; x++) {
       ...
       */
      rr = *tmpa >> ((tfm_fp_digit)(DIGIT_BIT - 1));

      /* now shift up this digit, add in the carry [from the previous] */
      *tmpb++ = ((*tmpa++ << ((tfm_fp_digit)1)) | r);
...
{
   tfm_fp_digit carry, carrytmp, shift;
   int x;
   ...
   if (c->alloc < (c->used + b / DIGIT_BIT + 1))
     if (tfm_fp_grow(c, c->used + b / DIGIT_BIT + 1))
       return TFM_FP_MEM;

   /* handle whole digits */
   if (b >= DIGIT_BIT) {
     if (tfm_fp_lshd(c, b/DIGIT_BIT))
       return TFM_FP_MEM;
   }
   b %= DIGIT_BIT;
...
#define MULADD(i, j)                                                              \
   do { tfm_fp_word t;                                                                \
   t = (tfm_fp_word)c0 + ((tfm_fp_word)i) * ((tfm_fp_word)j); c0 = t;                         \
   t = (tfm_fp_word)c1 + (t >> DIGIT_BIT);            c1 = t; c2 += t >> DIGIT_BIT;   \
   } while (0);
...
int tfm_fp_mul_comba(tfm_fp_int *A, tfm_fp_int *B, tfm_fp_int *C)
{
   ...
   tfm_fp_digit  c0, c1, c2, *tmpx, *tmpy;
   tfm_fp_int    tmp, *dst;
   ...
   if (A == C || B == C) {
     if (tfm_fp_init_size(C->stack, &tmp, pa))
       return TFM_FP_MEM;
      dst = &tmp;
   } else {
      tfm_fp_zeroise(C);
...
{
   tfm_fp_digit c0, c1, c2, at[40];

   SILC_LOG_DEBUG(("Start"));
   memcpy(at, A->dp, 20 * sizeof(tfm_fp_digit));
   memcpy(at+20, B->dp, 20 * sizeof(tfm_fp_digit));
   COMBA_START;
...
{
   tfm_fp_digit c0, c1, c2, at[48];

   SILC_LOG_DEBUG(("Start"));
   memcpy(at, A->dp, 24 * sizeof(tfm_fp_digit));
   memcpy(at+24, B->dp, 24 * sizeof(tfm_fp_digit));
   COMBA_START;
...
{
   tfm_fp_digit c0, c1, c2, at[56];

   SILC_LOG_DEBUG(("Start"));
   memcpy(at, A->dp, 28 * sizeof(tfm_fp_digit));
   memcpy(at+28, B->dp, 28 * sizeof(tfm_fp_digit));
   COMBA_START;
...
{
   tfm_fp_digit c0, c1, c2, at[64];
   int out_size;

   SILC_LOG_DEBUG(("Start"));
   out_size = A->used + B->used;
   memcpy(at, A->dp, 32 * sizeof(tfm_fp_digit));
   memcpy(at+32, B->dp, 32 * sizeof(tfm_fp_digit));
   COMBA_START;
   ...
   if (out_size <= 40) { COMBA_STORE2(C->dp[39]); C->used = 40; C->sign = A->sign ^ B->sign; tfm_fp_clamp(C); COMBA_FINI; return; }
   ...
   if (out_size <= 48) { COMBA_STORE2(C->dp[47]); C->used = 48; C->sign = A->sign ^ B->sign; tfm_fp_clamp(C); COMBA_FINI; return; }
   ...
   if (out_size <= 56) { COMBA_STORE2(C->dp[55]); C->used = 56; C->sign = A->sign ^ B->sign; tfm_fp_clamp(C); COMBA_FINI; return; }
...
{
   tfm_fp_digit c0, c1, c2, at[96];

   SILC_LOG_DEBUG(("Start"));
   memcpy(at, A->dp, 48 * sizeof(tfm_fp_digit));
   memcpy(at+48, B->dp, 48 * sizeof(tfm_fp_digit));
   COMBA_START;
...
{
   tfm_fp_digit c0, c1, c2, at[128];

   SILC_LOG_DEBUG(("Start"));
   memcpy(at, A->dp, 64 * sizeof(tfm_fp_digit));
   memcpy(at+64, B->dp, 64 * sizeof(tfm_fp_digit));
   COMBA_START;
...
{
   tfm_fp_digit c0, c1, c2, at[32];
   SILC_LOG_DEBUG(("Start"));
   switch (MAX(A->used, B->used)) {

   case 1:
      memcpy(at, A->dp, 1 * sizeof(tfm_fp_digit));
      memcpy(at+1, B->dp, 1 * sizeof(tfm_fp_digit));
      COMBA_START;
   ...
   case 2:
      memcpy(at, A->dp, 2 * sizeof(tfm_fp_digit));
      memcpy(at+2, B->dp, 2 * sizeof(tfm_fp_digit));
      COMBA_START;
   ...
   case 3:
      memcpy(at, A->dp, 3 * sizeof(tfm_fp_digit));
      memcpy(at+3, B->dp, 3 * sizeof(tfm_fp_digit));
      COMBA_START;
   ...
   case 4:
      memcpy(at, A->dp, 4 * sizeof(tfm_fp_digit));
      memcpy(at+4, B->dp, 4 * sizeof(tfm_fp_digit));
      COMBA_START;
   ...
   case 5:
      memcpy(at, A->dp, 5 * sizeof(tfm_fp_digit));
      memcpy(at+5, B->dp, 5 * sizeof(tfm_fp_digit));
      COMBA_START;
   ...
   case 6:
      memcpy(at, A->dp, 6 * sizeof(tfm_fp_digit));
      memcpy(at+6, B->dp, 6 * sizeof(tfm_fp_digit));
      COMBA_START;
   ...
   case 7:
      memcpy(at, A->dp, 7 * sizeof(tfm_fp_digit));
      memcpy(at+7, B->dp, 7 * sizeof(tfm_fp_digit));
      COMBA_START;
   ...
   case 8:
      memcpy(at, A->dp, 8 * sizeof(tfm_fp_digit));
      memcpy(at+8, B->dp, 8 * sizeof(tfm_fp_digit));
      COMBA_START;
   ...
   case 9:
      memcpy(at, A->dp, 9 * sizeof(tfm_fp_digit));
      memcpy(at+9, B->dp, 9 * sizeof(tfm_fp_digit));
      COMBA_START;
   ...
   case 10:
      memcpy(at, A->dp, 10 * sizeof(tfm_fp_digit));
      memcpy(at+10, B->dp, 10 * sizeof(tfm_fp_digit));
      COMBA_START;
   ...
   case 11:
      memcpy(at, A->dp, 11 * sizeof(tfm_fp_digit));
      memcpy(at+11, B->dp, 11 * sizeof(tfm_fp_digit));
      COMBA_START;
   ...
   case 12:
      memcpy(at, A->dp, 12 * sizeof(tfm_fp_digit));
      memcpy(at+12, B->dp, 12 * sizeof(tfm_fp_digit));
      COMBA_START;
   ...
   case 13:
      memcpy(at, A->dp, 13 * sizeof(tfm_fp_digit));
      memcpy(at+13, B->dp, 13 * sizeof(tfm_fp_digit));
      COMBA_START;
   ...
   case 14:
      memcpy(at, A->dp, 14 * sizeof(tfm_fp_digit));
      memcpy(at+14, B->dp, 14 * sizeof(tfm_fp_digit));
      COMBA_START;
   ...
   case 15:
      memcpy(at, A->dp, 15 * sizeof(tfm_fp_digit));
      memcpy(at+15, B->dp, 15 * sizeof(tfm_fp_digit));
      COMBA_START;
   ...
   case 16:
      memcpy(at, A->dp, 16 * sizeof(tfm_fp_digit));
      memcpy(at+16, B->dp, 16 * sizeof(tfm_fp_digit));
      COMBA_START;
...
int tfm_fp_mul_d(tfm_fp_int *a, tfm_fp_digit b, tfm_fp_int *c)
{
   ...
   w       = 0;
   for (x = 0; x < a->used; x++) {
       w         = ((tfm_fp_word)a->dp[x]) * ((tfm_fp_word)b) + w;
...
int tfm_fp_radix_size(tfm_fp_int *a, int radix, int *size)
{
  ...
  *size = 0;
  ...
  if (radix == 2) {
    *size = tfm_fp_count_bits(a) + (a->sign ? 1 : 0) + 1;
    return TFM_FP_OKAY;
  }
  ...
  if (tfm_fp_init_copy(&t, a, a->stack))
    return TFM_FP_MEM;

  /* if it is negative output a - */
  if (t.sign == TFM_FP_NEG) {
    (*size)++;
    t.sign = TFM_FP_ZPOS;
  }

  digs = 0;
  while (tfm_fp_iszero (&t) == TFM_FP_NO) {
    if (tfm_fp_div_d (&t, (tfm_fp_digit) radix, &t, &d)) {
      tfm_fp_zero(&t);
      return TFM_FP_MEM;
    }
    (*size)++;
...
const char *tfm_fp_s_rmap = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz+/";
...
int tfm_fp_read_radix(tfm_fp_int *a, char *str, int radix)
{
  ...
  while (*str) {
     ...
     */
    if (y < radix) {
      if (tfm_fp_mul_d (a, (tfm_fp_digit) radix, a))
	return TFM_FP_MEM;
      if (tfm_fp_add_d (a, (tfm_fp_digit) y, a))
	return TFM_FP_MEM;
    } else {
      break;
    }
...
void tfm_fp_read_unsigned_bin(tfm_fp_int *a, unsigned char *b, int c)
{
  ...
  /* If we know the endianness of this architecture, and we're using
     32-bit tfm_fp_digits, we can optimize this */
#if (defined(ENDIAN_LITTLE) || defined(ENDIAN_BIG)) && !defined(TFM_FP_64BIT)
...
#error Both ENDIAN_LITTLE and ENDIAN_BIG defined.
#endif
  {
     ...
     if ((unsigned)c > (TFM_FP_SIZE * sizeof(tfm_fp_digit))) {
        int excess = c - (TFM_FP_SIZE * sizeof(tfm_fp_digit));
        c -= excess;
        b += excess;
     }
     a->used = (c + sizeof(tfm_fp_digit) - 1)/sizeof(tfm_fp_digit);
     /* read the bytes in */
#ifdef ENDIAN_BIG
     {
...
{
  return 1 + tfm_fp_unsigned_bin_size (a);
}
	  ...
	  if (B->alloc < 128 + 1)
	    if (tfm_fp_grow(B, 128 + 1))
	      return TFM_FP_MEM;
           ...
           tfm_fp_sqr_comba64(A,B);
           return TFM_FP_OKAY;
        }
#endif

	if (B->alloc < A->used + A->used + 1)
	  if (tfm_fp_grow(B, A->used + A->used + 1))
	    return TFM_FP_MEM;

	return tfm_fp_sqr_comba(A, B);
}
...
#define SQRADD(i, j)                                 \
   do { tfm_fp_word t;                                   \
   t = c0 + ((tfm_fp_word)i) * ((tfm_fp_word)j);  c0 = t;    \
...
#define SQRADD2(i, j)                                                 \
   do { tfm_fp_word t;                                                    \
   t  = ((tfm_fp_word)i) * ((tfm_fp_word)j);                                  \
   tt = (tfm_fp_word)c0 + t;                 c0 = tt;                              \
   tt = (tfm_fp_word)c1 + (tt >> DIGIT_BIT); c1 = tt; c2 += tt >> DIGIT_BIT;       \
   tt = (tfm_fp_word)c0 + t;                 c0 = tt;                              \
   tt = (tfm_fp_word)c1 + (tt >> DIGIT_BIT); c1 = tt; c2 += tt >> DIGIT_BIT;       \
   } while (0);
...
#define SQRADDSC(i, j)                                                         \
   do { tfm_fp_word t;                                                             \
      t =  ((tfm_fp_word)i) * ((tfm_fp_word)j);                                        \
      sc0 = (tfm_fp_digit)t; sc1 = (t >> DIGIT_BIT); sc2 = 0;                      \
   } while (0);

#define SQRADDAC(i, j)                                                         \
   do { tfm_fp_word t;                                                             \
   t = sc0 + ((tfm_fp_word)i) * ((tfm_fp_word)j);  sc0 = t;                            \
   t = sc1 + (t >> DIGIT_BIT);             sc1 = t; sc2 += t >> DIGIT_BIT;     \
   } while (0);

#define SQRADDDB                                                               \
   do { tfm_fp_word t;                                                             \
   t = ((tfm_fp_word)sc0) + ((tfm_fp_word)sc0) + c0; c0 = t;                                                 \
   t = ((tfm_fp_word)sc1) + ((tfm_fp_word)sc1) + c1 + (t >> DIGIT_BIT); c1 = t;                              \
   c2 = c2 + ((tfm_fp_word)sc2) + ((tfm_fp_word)sc2) + (t >> DIGIT_BIT);                                     \
   } while (0);
...
{
   tfm_fp_digit *a, b[40], c0, c1, c2, sc0, sc1, sc2;
#ifdef TFM_ISO
   tfm_fp_word tt;
#endif
   ...
   B->used = 40;
   B->sign = TFM_FP_ZPOS;
   memcpy(B->dp, b, 40 * sizeof(tfm_fp_digit));
   tfm_fp_clamp(B);
}
#endif
...
{
   tfm_fp_digit *a, b[48], c0, c1, c2, sc0, sc1, sc2;
#ifdef TFM_ISO
   tfm_fp_word tt;
#endif
   ...
   B->used = 48;
   B->sign = TFM_FP_ZPOS;
   memcpy(B->dp, b, 48 * sizeof(tfm_fp_digit));
   tfm_fp_clamp(B);
}
#endif
...
{
   tfm_fp_digit *a, b[56], c0, c1, c2, sc0, sc1, sc2;
#ifdef TFM_ISO
   tfm_fp_word tt;
#endif
   ...
   B->used = 56;
   B->sign = TFM_FP_ZPOS;
   memcpy(B->dp, b, 56 * sizeof(tfm_fp_digit));
   tfm_fp_clamp(B);
}
#endif
...
{
   tfm_fp_digit *a, b[64], c0, c1, c2, sc0, sc1, sc2;
#ifdef TFM_ISO
   tfm_fp_word tt;
#endif
   ...
   B->used = 64;
   B->sign = TFM_FP_ZPOS;
   memcpy(B->dp, b, 64 * sizeof(tfm_fp_digit));
   tfm_fp_clamp(B);
}
#endif
...
{
   tfm_fp_digit *a, b[96], c0, c1, c2, sc0, sc1, sc2;
#ifdef TFM_ISO
   tfm_fp_word tt;
#endif
   ...
   B->used = 96;
   B->sign = TFM_FP_ZPOS;
   memcpy(B->dp, b, 96 * sizeof(tfm_fp_digit));
   tfm_fp_clamp(B);
}
#endif
...
{
   tfm_fp_digit *a, b[128], c0, c1, c2, sc0, sc1, sc2;
#ifdef TFM_ISO
   tfm_fp_word tt;
#endif
   ...
   B->used = 128;
   B->sign = TFM_FP_ZPOS;
   memcpy(B->dp, b, 128 * sizeof(tfm_fp_digit));
   tfm_fp_clamp(B);
}
#endif
...
{
   tfm_fp_digit *a, b[32], c0, c1, c2, sc0, sc1, sc2;
#ifdef TFM_ISO
   tfm_fp_word   tt;
#endif
   SILC_LOG_DEBUG(("Start"));
   ...
   case 1:
      ...
      B->used = 2;
      B->sign = TFM_FP_ZPOS;
      memcpy(B->dp, b, 2 * sizeof(tfm_fp_digit));
      tfm_fp_clamp(B);
      break;
   ...
   case 2:
      ...
      B->used = 4;
      B->sign = TFM_FP_ZPOS;
      memcpy(B->dp, b, 4 * sizeof(tfm_fp_digit));
      tfm_fp_clamp(B);
      break;
   ...
   case 3:
      ...
      B->used = 6;
      B->sign = TFM_FP_ZPOS;
      memcpy(B->dp, b, 6 * sizeof(tfm_fp_digit));
      tfm_fp_clamp(B);
      break;
   ...
   case 4:
      ...
      B->used = 8;
      B->sign = TFM_FP_ZPOS;
      memcpy(B->dp, b, 8 * sizeof(tfm_fp_digit));
      tfm_fp_clamp(B);
      break;
   ...
   case 5:
      ...
      B->used = 10;
      B->sign = TFM_FP_ZPOS;
      memcpy(B->dp, b, 10 * sizeof(tfm_fp_digit));
      tfm_fp_clamp(B);
      break;
   ...
   SQRADDSC(a[0], a[5]); SQRADDAC(a[1], a[4]); SQRADDAC(a[2], a[3]); SQRADDDB;
      ...
      B->used = 12;
      B->sign = TFM_FP_ZPOS;
      memcpy(B->dp, b, 12 * sizeof(tfm_fp_digit));
      tfm_fp_clamp(B);
      break;
   ...
   SQRADDSC(a[1], a[6]); SQRADDAC(a[2], a[5]); SQRADDAC(a[3], a[4]); SQRADDDB;
      ...
      B->used = 14;
      B->sign = TFM_FP_ZPOS;
      memcpy(B->dp, b, 14 * sizeof(tfm_fp_digit));
      tfm_fp_clamp(B);
      break;
   ...
   SQRADDSC(a[2], a[7]); SQRADDAC(a[3], a[6]); SQRADDAC(a[4], a[5]); SQRADDDB;
      ...
      B->used = 16;
      B->sign = TFM_FP_ZPOS;
      memcpy(B->dp, b, 16 * sizeof(tfm_fp_digit));
      tfm_fp_clamp(B);
      break;
   ...
   SQRADDSC(a[3], a[8]); SQRADDAC(a[4], a[7]); SQRADDAC(a[5], a[6]); SQRADDDB;
      ...
      B->used = 18;
      B->sign = TFM_FP_ZPOS;
      memcpy(B->dp, b, 18 * sizeof(tfm_fp_digit));
      tfm_fp_clamp(B);
      break;
   ...
   SQRADDSC(a[4], a[9]); SQRADDAC(a[5], a[8]); SQRADDAC(a[6], a[7]); SQRADDDB;
      ...
      B->used = 20;
      B->sign = TFM_FP_ZPOS;
      memcpy(B->dp, b, 20 * sizeof(tfm_fp_digit));
      tfm_fp_clamp(B);
      break;
   ...
   SQRADDSC(a[5], a[10]); SQRADDAC(a[6], a[9]); SQRADDAC(a[7], a[8]); SQRADDDB;
      ...
      B->used = 22;
      B->sign = TFM_FP_ZPOS;
      memcpy(B->dp, b, 22 * sizeof(tfm_fp_digit));
      tfm_fp_clamp(B);
      break;
   ...
   SQRADDSC(a[6], a[11]); SQRADDAC(a[7], a[10]); SQRADDAC(a[8], a[9]); SQRADDDB;
      ...
      B->used = 24;
      B->sign = TFM_FP_ZPOS;
      memcpy(B->dp, b, 24 * sizeof(tfm_fp_digit));
      tfm_fp_clamp(B);
      break;
   ...
   SQRADDSC(a[7], a[12]); SQRADDAC(a[8], a[11]); SQRADDAC(a[9], a[10]); SQRADDDB;
      ...
      B->used = 26;
      B->sign = TFM_FP_ZPOS;
      memcpy(B->dp, b, 26 * sizeof(tfm_fp_digit));
      tfm_fp_clamp(B);
      break;
   ...
   SQRADDSC(a[8], a[13]); SQRADDAC(a[9], a[12]); SQRADDAC(a[10], a[11]); SQRADDDB;
      ...
      B->used = 28;
      B->sign = TFM_FP_ZPOS;
      memcpy(B->dp, b, 28 * sizeof(tfm_fp_digit));
      tfm_fp_clamp(B);
      break;
   ...
   SQRADDSC(a[9], a[14]); SQRADDAC(a[10], a[13]); SQRADDAC(a[11], a[12]); SQRADDDB;
      ...
      B->used = 30;
      B->sign = TFM_FP_ZPOS;
      memcpy(B->dp, b, 30 * sizeof(tfm_fp_digit));
      tfm_fp_clamp(B);
      break;
   ...
   SQRADDSC(a[10], a[15]); SQRADDAC(a[11], a[14]); SQRADDAC(a[12], a[13]); SQRADDDB;
      ...
      B->used = 32;
      B->sign = TFM_FP_ZPOS;
      memcpy(B->dp, b, 32 * sizeof(tfm_fp_digit));
      tfm_fp_clamp(B);
      break;
}
}
...
int tfm_fp_sub(tfm_fp_int *a, tfm_fp_int *b, tfm_fp_int *c)
{
  ...
  if (sa != sb) {
    ...
    if (tfm_fp_cmp_mag (a, b) != TFM_FP_LT) {
      ...
      /* the first number. */
      c->sign = (sa == TFM_FP_ZPOS) ? TFM_FP_NEG : TFM_FP_ZPOS;
...
{
  tfm_fp_to_unsigned_bin (a, b + 1);
  b[0] = (unsigned char) ((a->sign == TFM_FP_ZPOS) ? 0 : 1);
}
...
int tfm_fp_toradix(tfm_fp_int *a, char *str, int radix)
{
   ...
   */
  tfm_fp_reverse ((unsigned char *)_s, digs);
...
int s_tfm_fp_add(tfm_fp_int *a, tfm_fp_int *b, tfm_fp_int *c)
{
  ...
  /* Grow when needed */
  if (c->alloc < c->used + 1)
    if (tfm_fp_grow(c, c->used + 1))
      return TFM_FP_MEM;

  t = 0;
  for (x = 0; x < Y->used; x++) {
      t         += ((tfm_fp_word)X->dp[x]) + ((tfm_fp_word)Y->dp[x]);
      c->dp[x]   = (tfm_fp_digit)t;
      t        >>= DIGIT_BIT;
  }
  for (; x < X->used; x++) {
      t         += ((tfm_fp_word)X->dp[x]);
      c->dp[x]   = (tfm_fp_digit)t;
      t        >>= DIGIT_BIT;
  }
  if (t != 0 && x < c->alloc) {
     c->dp[c->used++] = (tfm_fp_digit)t;
     ++x;
  }
...
int s_tfm_fp_sub(tfm_fp_int *a, tfm_fp_int *b, tfm_fp_int *c)
{
  ...
  /* Grow when needed */
  if (c->alloc < a->used)
    if (tfm_fp_grow(c, a->used))
      return TFM_FP_MEM;

  t       = 0;
  for (x = 0; x < oldbused; x++) {
     t         = ((tfm_fp_word)a->dp[x]) - (((tfm_fp_word)b->dp[x]) + t);
     c->dp[x]  = (tfm_fp_digit)t;
     t         = (t >> DIGIT_BIT)&1;
  }
  for (; x < a->used; x++) {
     t         = ((tfm_fp_word)a->dp[x]) - t;
     c->dp[x]  = (tfm_fp_digit)t;
     t         = (t >> DIGIT_BIT);
   }
  for (; x < oldused; x++) {
...
int tfm_fp_sqr_comba(tfm_fp_int *A, tfm_fp_int *B)
{
  ...
  if (A == B) {
    if (tfm_fp_init_size(B->stack, &tmp, pa))
      return TFM_FP_MEM;
    dst = &tmp;
  } else {
    tfm_fp_zeroise(B);
...
int tfm_fp_xor(tfm_fp_int * a, tfm_fp_int * b, tfm_fp_int * c)
{
  ...
  if (a->used > b->used) {
    /* If result doesn't have stack, assure it doesn't get one */
    if ((res = tfm_fp_init_copy (&t, a, c->stack ? a->stack : NULL))) {
      return res;
    }
    px = b->used;
    x = b;
  } else {
    if ((res = tfm_fp_init_copy (&t, b, c->stack ? b->stack : NULL))) {
      return res;
    }
    px = a->used;
    x = a;
...
int tfm_fp_and(tfm_fp_int * a, tfm_fp_int * b, tfm_fp_int * c)
{
  ...
  /* zero digits above the last from the smallest tfm_fp_int */
  for (; ix < t.used; ix++) {
    t.dp[ix] = 0;
  }
...
int tfm_fp_expt_d(tfm_fp_int * a, tfm_fp_digit b, tfm_fp_int * c)
{
  ...
  if ((res = tfm_fp_init_copy (&g, a, a->stack)) != TFM_FP_OKAY)
    return res;
  ...
  for (x = 0; x < (int) DIGIT_BIT; x++) {
    /* square */
    if ((res = tfm_fp_sqr (c, c)) != TFM_FP_OKAY) {
      tfm_fp_zero (&g);
      return res;
    }

    /* if the bit is set multiply */
    if ((b & (tfm_fp_digit) (((tfm_fp_digit)1) << (DIGIT_BIT - 1))) != 0) {
      if ((res = tfm_fp_mul(c, &g, c)) != TFM_FP_OKAY) {
         tfm_fp_zero (&g);
         return res;
      }
    }
...
int tfm_fp_sqrt(tfm_fp_int *arg, tfm_fp_int *ret)
{
  ...
  /* must be positive */
  if (arg->sign == TFM_FP_NEG) {
    return TFM_FP_VAL;
  }

  /* easy out */
  if (tfm_fp_iszero(arg) == TFM_FP_YES) {
    tfm_fp_zero(ret);
    return TFM_FP_OKAY;
  }

  /* If result doesn't have stack, assure it doesn't get one */
  if ((res = tfm_fp_init_copy(&t1, arg, ret->stack ? arg->stack : NULL)))
    return res;

  tfm_fp_init(&t2);

  /* First approx. (not very bad for large arg) */
  tfm_fp_rshd (&t1,t1.used/2);

  /* t1 > 0  */
  if ((res = tfm_fp_div(arg,&t1,&t2,NULL)) != TFM_FP_OKAY) {
    goto E1;
  }
  if ((res = tfm_fp_add(&t1,&t2,&t1)) != TFM_FP_OKAY) {
    goto E1;
  }
  if ((res = tfm_fp_div_2(&t1,&t1)) != TFM_FP_OKAY) {
    goto E1;
  }
